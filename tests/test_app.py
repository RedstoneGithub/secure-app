...
class BaseTestCase(unittest.TestCase):
    ...
    def _login(self, username="alice", password="AlicePass12!", follow_redirects=True):
    ...
    def _upload(
        ...
        after = set(f for f in os.listdir("data") if f.endswith(".enc"))
    ...
    def _enc_files(self):
        return [
            f"{doc_id}.enc"
            for doc_id in self._created_docs
            if os.path.exists(os.path.join("data", f"{doc_id}.enc"))
        ]
...
class TestAccessControl(BaseTestCase):
    ...
    def test_non_owner_cannot_share(self):
        ...
        self.client.post(
            f"/documents/share/{doc_id}", data={"username": "target", "role": "viewer"}
        )
    ...
    def test_admin_can_lock_user(self):
        ...
        self.client.post(
            "/admin/users/u_victim/lock", data={"action": "lock"}, follow_redirects=True
        )
        users = self._get_users()
...
class TestInputValidation(BaseTestCase):
    ...
    def test_upload_allowed_pdf(self):
        ...
        self._login()
        self._upload(
            filename="doc.pdf", content=b"%PDF fake", mimetype="application/pdf"
        )
        self.assertGreater(
    ...
    def test_share_invalid_role_rejected(self):
        ...
        doc_id = enc_files[0].replace(".enc", "")
        self.client.post(
            f"/documents/share/{doc_id}", data={"username": "target", "role": "admin"}
        )
        doc = encrypted_storage.load_encrypted(f"data/{doc_id}.enc")
        self.assertNotIn("u_target", doc.get("shared_with", {}))
    ...
    def test_share_self_rejected(self):
        ...
        doc_id = enc_files[0].replace(".enc", "")
        self.client.post(
            f"/documents/share/{doc_id}", data={"username": "owner", "role": "viewer"}
        )
        doc = encrypted_storage.load_encrypted(f"data/{doc_id}.enc")
...
class TestSecurityHeaders(BaseTestCase):
    ...
    def test_csp_default_src_self(self):
        self.assertIn(
            "default-src 'self'", self._h().get("Content-Security-Policy", "")
        )
    ...
    def test_hsts_includes_subdomains(self):
        self.assertIn(
            "includeSubDomains", self._h().get("Strict-Transport-Security", "")
        )
...
class TestDocumentFeatures(BaseTestCase):
    def test_first_upload_version_is_1(self):
        ...
        enc_files = self._enc_files()
        doc = encrypted_storage.load_encrypted(f"data/{enc_files[0]}")
        self.assertEqual(doc.get("version"), 1)

    def test_uploaded_doc_shows_immediately_on_documents_page(self):
        self._seed_user()
        self._login()
        resp = self._upload(filename="fresh.txt", content=b"fresh content")
        self.assertEqual(resp.status_code, 200)
        self.assertIn(b"fresh.txt", resp.data)
    ...
    def test_version_history_page_accessible(self):
        ...
        doc_id = enc_files[0].replace(".enc", "")
        self.assertEqual(
            self.client.get(f"/documents/versions/{doc_id}").status_code, 200
        )
    ...
    def test_owner_can_download_previous_version(self):
        ...
        enc_files = self._enc_files()
        doc_id = enc_files[0].replace(".enc", "")
        resp = self.client.get(f"/documents/versions/{doc_id}/download/1")
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data, b"v1 content")
        self.assertIn("report_v1.txt", resp.headers.get("Content-Disposition", ""))

    def test_owner_can_restore_previous_version(self):
        self._seed_user()
        self._login()
        self._upload(filename="report.txt", content=b"v1 content")
        self._upload(filename="report.txt", content=b"v2 content")
        ...
        doc_id = enc_files[0].replace(".enc", "")
        resp = self.client.post(
            f"/documents/versions/{doc_id}/restore/1", follow_redirects=True
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_share_editor_recorded(self):
        ...
        doc_id = enc_files[0].replace(".enc", "")
        self.client.post(
            f"/documents/share/{doc_id}", data={"username": "collab", "role": "editor"}
        )
        doc = encrypted_storage.load_encrypted(f"data/{doc_id}.enc")
    ...
    def test_shared_doc_visible_to_recipient(self):
        ...
        doc_id = enc_files[0].replace(".enc", "")
        self.client.post(
            f"/documents/share/{doc_id}", data={"username": "reader", "role": "viewer"}
        )
        self.client.get("/logout")
    ...
    def test_viewer_can_open_shared_doc(self):
        self._seed_user(username="owner", uid="u_owner")
        self._seed_user(username="viewer", uid="u_viewer")
        self._login(username="owner")
        self._upload(filename="shared.txt", content=b"shared content")
        ...
        self.client.get("/logout")
        self._login(username="viewer")
        resp = self.client.get(f"/documents/view/{doc_id}")
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data, b"shared content")
